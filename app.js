...
function renderInviteLinks() {
  const open = state.invites.filter((i) => !i.used);
  if (!open.length) return '<div class="small">No open invite links.</div>';
  return `<div class="small">${open.map((i) => { const link = `${location.origin}${location.pathname}#invite=${encodeURIComponent(i.token)}`; return `${i.clientEmail}: <a href="${link}" target="_blank">${link}</a>`; }).join('<br/>')}</div>`;
}
