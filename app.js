...
function ensureStateCollections() {
  if (!Array.isArray(state.contract_signatures)) state.contract_signatures = [];
}

const KEYLINE_LAUNCH_AGREEMENT = `KEYLINE STUDIOS SERVICE AGREEMENT (LAUNCH WEBSITE)
This Service Agreement (the “Agreement”) is entered into between Keyline Studios LLC and the Client
1. PROJECT SCOPE
The Studio will provide the services described in Exhibit A (Scope and Deliverables) attached to this Agreement.
Staging / Preview Use: Any preview or staging links, password-protected pages, or demo versions are for review only and may not be used as a live production site until the final payment clears.
Out of Scope: Anything not explicitly listed in Exhibit A is out of scope and requires a written change order.
2. FEES AND PAYMENT TERMS
Payment Schedule:
Deposit: 40% due before work begins
Final Payment: 60% due before launch on the Client’s domain and before delivery of any final production-ready assets
Invoicing and Methods: Invoices are issued via Stripe. Stripe may accept cards, Apple Pay, and other supported methods. ACH or wire may be available upon request.
Late Payments / Pause: Late or unpaid invoices may pause work and delay the timeline. If the final payment is not received, the Studio may remove the preview/staging site.
3. TIMELINE AND CLIENT RESPONSIBILITIES
Estimated Timeline: The Studio will begin after the deposit is received and the Client provides required materials (see Exhibit A). The timeline may shift if the Client delays approvals, content, or feedback.
Client Responsibilities: The Client agrees to provide requested assets and feedback in a timely manner, including logo/brand assets, product photos/copy, contact email for forms, and domain or DNS/Cloudflare access needed for connection.
4. REVISIONS
The Launch Website includes two (2) revision rounds as defined in Exhibit A. A revision round means one consolidated set of edits submitted at one time. Additional revisions, new sections, or scope changes are billed separately at $100/hour and must be approved in writing before the Studio proceeds.
5. CHANGE ORDERS (SCOPE CHANGES)
Any work outside Exhibit A requires a written change order that includes pricing and timeline adjustments.
6. DELIVERY, LAUNCH, AND ACCESS
The Studio may host the site on a Studio-controlled preview link during development. The Studio will connect the Client’s custom domain and publish the site after final payment clears. If final payment is not received, the Studio may take the site offline and revoke preview access.
7. INTELLECTUAL PROPERTY
Ownership Until Paid: All designs, drafts, and work products remain the property of the Studio until all invoices are paid in full.
Rights After Full Payment: After full payment clears, the Client receives ownership rights to final deliverables in Exhibit A, excluding Studio pre-existing tools/templates/processes.
Client Materials: The Client retains ownership of their provided logos/photos/trademarks/content and represents they have rights to use them.
8. THIRD-PARTY SERVICES
The project may involve third-party services (domain registrar, DNS, Cloudflare, email provider, hosting platform). The Client is responsible for related fees. The Studio is not responsible for third-party outages, policy changes, or interruptions.
9. CONFIDENTIALITY AND PORTFOLIO
Both parties will keep non-public project information confidential. Unless the Client requests a written NDA, the Studio may display the final public website in portfolio after launch.
10. WARRANTY AND SUPPORT
The Studio provides a 14-day bug-fix window after launch for Studio-caused implementation issues. Ongoing maintenance/updates/new work are not included unless purchased separately.
11. CANCELLATION AND TERMINATION
Deposit is non-refundable. If Client cancels after work starts, deposit is retained and Client must pay for completed work to date. For nonpayment, Studio may stop work, revoke previews, and terminate Agreement. No rights transfer until full payment is received.
12. LIMITATION OF LIABILITY
The Studio is not liable for indirect/incidental/special/consequential/punitive damages. Total liability will not exceed amounts paid by Client under this Agreement.
13. GOVERNING LAW AND DISPUTE RESOLUTION
This Agreement is governed by Florida law. Parties agree to attempt good-faith mediation before legal claim.
14. ACCEPTANCE AND SIGNATURE
This Agreement may be signed electronically and in counterparts. Electronic signatures are valid and enforceable.
...
async function boot() {
  ...
  ensureStateCollections();
      ensureStateCollections();
      if (!state.projects.some((project) => project.projectId === selectedProjectId)) {
  ...
  appNode.innerHTML = `
    <div class="auth glass">
      ...
      <form id="inviteForm" class="section">
        <div class="form-group"><label>Invite token</label><input name="token" value="${escapeAttr(readInviteTokenFromUrl())}" required /></div>
...
function renderContractAgreement(me, project) {
  appNode.innerHTML = `
    <div class="auth glass contract-box">
      ...
      <form id="contractSignForm" class="section">
        ...
        <div class="form-group"><label><input type="checkbox" name="agree" /> I have read and agree to this Service Agreement.</label></div>
        <div class="button-row"><button class="primary">Sign Agreement</button></div>
      </form>
      <button id="logoutBtn" class="secondary">Logout</button>
    </div>
  `;
  document.querySelector('#logoutBtn').onclick = logout;
  document.querySelector('#contractSignForm').onsubmit = async (e) => {
    e.preventDefault();
    const fd = new FormData(e.target);
    const typedName = String(fd.get('typedName') || '').trim();
    const agreed = fd.get('agree') === 'on';
    if (!typedName) return alert('Please type your full name.');
    if (!agreed) return alert('Please confirm agreement acceptance.');
...
function renderRoute(me) {
  if (me.role === 'client') {
    const project = getClientProject(me);
...
function renderProjectAdmin(me) {
  ...
  const client = getUser(p.clientId) || { name: p.clientEmail || 'Unassigned' };
  const signature = getContractSignature(p.clientId, p.projectId) || getContractSignatureByEmail(p.clientEmail, p.projectId);
  const signaturePanel = signature
    ? `<div class="small"><strong>Signed:</strong> ${fmt(signature.signedAt)}</div><div class="small"><strong>Signed by:</strong> ${escapeHtml(signature.typedName)} (${escapeHtml(signature.clientEmail || '')})</div><details><summary>View signed agreement details</summary><div class="small">Agreement: ${escapeHtml(signature.agreementTitle)} (${escapeHtml(signature.agreementVersion)})</div></details>`
    : '<div class="small">No signed agreement on file yet.</div>';
  return `<section class="section"><h2>Project Admin View</h2><div class="small">Metric filter: ${adminHudFilter}</div><div class="form-group project-picker-group"><label>Selected project</label><select id="adminProjectPicker">${source.map((project) => `<option value="${project.projectId}" ${project.projectId === selectedProjectId ? 'selected' : ''}>${escapeHtml(project.businessName)}</option>`).join('')}</select></div><form id="adminEditProject"><div class="form-grid"><div class="form-group"><label>Status</label><select name="status">${['Audit', 'Mockup', 'Build', 'Live'].map((status) => `<option ${status === p.status ? 'selected' : ''}>${status}</option>`).join('')}</select></div><div class="form-group"><label>Due date</label><input type="date" name="dueDate" value="${p.dueDate.slice(0, 10)}" /></div><div class="form-group"><label>Price</label><input name="maxPrice" value="${escapeAttr(p.maxPrice || '')}" placeholder="e.g. 5000"/></div><div class="form-group"><label>Assigned admin</label><select name="assignedAdminId">${state.users.filter((u) => u.role === 'admin').map((u) => `<option value="${u.uid}" ${u.uid === p.assignedAdminId ? 'selected' : ''}>${escapeHtml(u.name)}</option>`).join('')}</select></div><div class="form-group"><label>Next step</label><input name="nextStepText" value="${escapeAttr(p.nextStepText)}"/></div></div><div class="button-row"><button class="primary">Save changes</button></div></form></section><section class="section"><h3>Signed Contract</h3>${signaturePanel}</section><section class="section"><h3>Upload deliverable</h3><form id="addDeliverable"><div class="form-grid"><div class="form-group"><label>Title</label><input name="title" required /></div><div class="form-group"><label>Type</label><select name="type"><option value="mockup">mockup</option><option value="live">live</option></select></div><div class="form-group upload-card"><label>Upload file (optional)</label><input class="file-input" name="deliverableFile" type="file" /></div><div class="form-group"><label>URL (optional)</label><input name="url" /></div></div><div class="button-row"><button class="secondary">Add deliverable</button></div></form></section><section class="section"><h3>Client uploads (${escapeHtml(client?.name || '')})</h3>${state.uploads.filter((u) => u.projectId === p.projectId).map((u) => `<div>${escapeHtml(u.label)} · ${escapeHtml(u.fileType)} · <a href="${u.fileUrl}" target="_blank">Open</a></div>`).join('') || '<div class="small">None</div>'}</section>${renderMessages(p, me)}`;
}
...
function renderAdmins(me) { return `<section class="section"><h2>Admin Management</h2><form id="addAdmin"><div class="form-grid"><div class="form-group"><label>Email</label><input name="email" required /></div><div class="form-group"><label>Name</label><input name="name" required /></div></div><div class="small">This adds role metadata in Firestore state. Auth user must still be created in Firebase Auth.</div><div class="button-row"><button class="primary">Add admin</button></div></form></section><section class="section"><h3>Admins</h3>${state.users.filter((u) => u.role === 'admin').map((u) => `<div>${escapeHtml(u.name)} (${escapeHtml(u.email)}) <span class="small">· last active ${fmt(u.lastActiveAt)}</span> ${u.uid !== me.uid ? `<button class="secondary" data-remove-admin="${u.uid}">Remove</button>` : ''}</div>`).join('')}</section><section class="section"><h3>Reset user password</h3><p class="small">Verify user by email, name, and project name, then send reset email.</p><form id="adminResetPassword"><div class="form-grid"><div class="form-group"><label>User email</label><input name="email" type="email" required /></div><div class="form-group"><label>User name</label><input name="name" required /></div><div class="form-group"><label>Project name</label><input name="projectName" required /></div></div><div class="button-row"><button class="secondary">Verify + send reset email</button></div></form></section>`; }
...
function bindForms(me) {
  ...
  const createProjectInvite = document.querySelector('#createProjectInvite'); if (createProjectInvite) createProjectInvite.onsubmit = (e) => { e.preventDefault(); const fd = new FormData(e.target); const email = String(fd.get('clientEmail')).toLowerCase(); const existing = state.users.find((u) => u.email.toLowerCase() === email); const token = `INV-${Math.random().toString(36).slice(2, 10).toUpperCase()}`; const projectId = uid('proj'); state.invites.push({ token, clientEmail: email, expiresAt: addHours(24), used: false }); state.projects.push({ projectId, clientId: existing?.uid || '', clientEmail: email, businessName: String(fd.get('businessName')), status: 'Audit', dueDate: new Date(String(fd.get('dueDate'))).toISOString(), startDate: now(), nextStepText: String(fd.get('nextStepText')), maxPrice: String(fd.get('maxPrice') || ''), assignedAdminId: me.uid, createdAt: now(), updatedAt: now(), lastActivityAt: now(), statusHistory: [{ status: 'Audit', at: now() }] }); if (!existing) { state.users.push({ uid: uid('pending'), email, role: 'client', name: String(fd.get('clientName')), createdAt: now(), lastActiveAt: now(), firstLogin: true, notifications: true }); } logEvent(me.uid, me.role, 'project.create', 'projects', projectId, projectId); persist(); latestInviteLink = buildInviteLink(token);
  alert('Invite link created. Use copy link button.'); render(); };
  const copyInviteLink = document.querySelector('#copyInviteLink'); if (copyInviteLink) copyInviteLink.onclick = async () => { try { await navigator.clipboard.writeText(latestInviteLink); alert('Invite link copied.'); } catch { const input = document.querySelector('#latestInviteLink'); if (input) { input.select(); document.execCommand('copy'); alert('Invite link copied.'); } } };
  const adminProjectPicker = document.querySelector('#adminProjectPicker'); if (adminProjectPicker) adminProjectPicker.onchange = (e) => { selectedProjectId = e.target.value; render(); };
  const adminEdit = document.querySelector('#adminEditProject'); if (adminEdit) adminEdit.onsubmit = (e) => { e.preventDefault(); const p = state.projects.find((x) => x.projectId === selectedProjectId); const fd = new FormData(e.target); const prevStatus = p.status; p.status = fd.get('status'); p.dueDate = new Date(fd.get('dueDate')).toISOString(); p.assignedAdminId = fd.get('assignedAdminId'); p.nextStepText = fd.get('nextStepText'); p.maxPrice = String(fd.get('maxPrice') || ''); p.updatedAt = now(); if (prevStatus !== p.status) p.statusHistory.push({ status: p.status, at: now() }); logEvent(me.uid, me.role, 'project.update', 'projects', p.projectId, p.projectId); persist(); render(); };
  const addDeliverable = document.querySelector('#addDeliverable'); if (addDeliverable) addDeliverable.onsubmit = async (e) => { e.preventDefault(); const fd = new FormData(e.target); const file = fd.get('deliverableFile'); let deliverableUrl = String(fd.get('url') || '').trim(); if (file && file.size > 0) deliverableUrl = await uploadFileToStorage(file, `projects/${selectedProjectId}/deliverables`); if (!deliverableUrl) return alert('Please upload a file or provide a URL.'); const d = { deliverableId: uid('del'), projectId: selectedProjectId, title: fd.get('title'), type: fd.get('type'), urls: [deliverableUrl], status: 'pending', createdAt: now() }; state.deliverables.push(d); logEvent(me.uid, me.role, 'deliverable.create', 'deliverables', d.deliverableId, selectedProjectId); persist(); render(); };
  ...
  const adminResetPassword = document.querySelector('#adminResetPassword'); if (adminResetPassword) adminResetPassword.onsubmit = async (e) => { e.preventDefault(); const fd = new FormData(e.target); const email = String(fd.get('email') || '').trim().toLowerCase(); const name = String(fd.get('name') || '').trim().toLowerCase(); const projectName = String(fd.get('projectName') || '').trim().toLowerCase(); const user = state.users.find((u) => u.role === 'client' && u.email.toLowerCase() === email && u.name.toLowerCase() === name); const project = state.projects.find((proj) => String(proj.businessName || '').toLowerCase() === projectName && (proj.clientId === user?.uid || (proj.clientEmail || '').toLowerCase() === email)); if (!user || !project) return alert('Verification failed. Please check email, name, and project name.'); try { await sendPasswordResetEmail(auth, user.email); logEvent(me.uid, me.role, 'user.password_reset_requested', 'users', user.uid, project.projectId); alert('Verification passed. Reset email sent.'); adminResetPassword.reset(); } catch (error) { alert(`Unable to send reset email: ${error.message}`); } };
...
function renderLatestInviteLink() {
  if (!latestInviteLink) return '<div class="small">No new invite link generated yet.</div>';
  return `<div class="form-grid"><div class="form-group"><label>Invite link</label><input id="latestInviteLink" value="${escapeAttr(latestInviteLink)}" readonly /></div></div><div class="button-row"><button type="button" id="copyInviteLink" class="secondary">Copy link</button></div>`;
}

function buildInviteLink(token) {
  const base = (publicAppUrl && String(publicAppUrl).trim()) ? String(publicAppUrl).replace(/\/$/, '') : `${location.origin}${location.pathname}`.replace(/\/$/, '');
...
function readInviteTokenFromUrl() {
  ...
  return decodeURIComponent(hash.replace('#invite=', ''));
}


function getClientProject(me) {
  return state.projects.find((project) => project.clientId === me.uid || (project.clientEmail && project.clientEmail.toLowerCase() === me.email.toLowerCase()));
}
...
function getContractSignatureByEmail(clientEmail, projectId) {
  ...
  return (state.contract_signatures || []).find((signature) => String(signature.clientEmail || '').toLowerCase() === String(clientEmail).toLowerCase() && signature.projectId === projectId) || null;
}
...
function projectRow(p) { return `<tr><td>${escapeHtml(p.businessName)}</td><td>${escapeHtml(p.status)}</td><td class="meta">${escapeHtml(p.maxPrice || '—')}</td><td class="meta">${fmt(p.dueDate)}</td><td class="meta">${isOverdueProject(p) ? 'Yes' : 'No'}</td><td class="meta">${projectWaiting(p) ? 'Yes' : 'No'}</td></tr>`; }
