...
function render() {
  ...
  appNode.innerHTML = `
    <div class="shell">
      <nav class="glass">
        ...
        ${me.role === 'admin' ? renderSidebarProjectPicker() : ''}
  ...
  const sidebarProjectPicker = appNode.querySelector('#sidebarProjectPicker');
  if (sidebarProjectPicker) sidebarProjectPicker.onchange = (e) => { selectedProjectId = e.target.value; render(); };
  ...
  appNode.innerHTML = `
    <div class="auth glass">
      ...
      <form id="inviteForm" class="section">
        ...
        <div class="form-group upload-card"><label>Project images (optional uploads)</label><input name="imageFiles" type="file" accept="image/*" multiple /></div>
...
function renderRoute(me) {
  if (me.role === 'client') {
    const project = state.projects.find((p) => p.clientId === me.uid || (p.clientEmail && p.clientEmail.toLowerCase() === me.email.toLowerCase()));
...
function renderSidebarProjectPicker() {
  if (!state.projects.length) return '';
  return `<div class="nav-project-picker"><label>Project</label><select id="sidebarProjectPicker">${state.projects.map((project) => `<option value="${project.projectId}" ${project.projectId === selectedProjectId ? 'selected' : ''}>${escapeHtml(project.businessName)}</option>`).join('')}</select></div>`;
}
